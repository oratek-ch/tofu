...
The TOFU5+ is the next-generation carrier board for the Raspberry Pi Compute Module 5 (CM5), designed to meet the evolving demands of industrial applications.

Built on the proven TOFU platform, the TOFU5+ enhances existing features, adds new functionalities, and maximizes the capabilities of the latest Compute Module. Maintaining the same compact footprint as the previous generation, this board introduces an updated and intelligent integration of displays, cameras, faster USB 3.0 ports, and two M.2 ports. These additions allow seamless use of SSDs and cellular modules, enabling a wide range of applications.

The TOFU5+ is engineered to operate over an extensive power range and offers numerous customization possibilities. With improved protection features, it delivers reliable performance across diverse and demanding scenarios.

![tofu5+_cm5](./_media/TOFU5+_CM5_FRONT.png)  
...
![tofu5+_back](./_media/TOFU5+_CM5_BACK.png)
M.2 connectivity (key B + key M) enables to fully take advantage of the new PCIe lane.

## Operating temperature
The CM5 is rated from -20°C to 85°C. All components on the TOFU board comply with this range.
Apart from connectors, all electronic components are rated for -40°C or below.

## Layout

![tofu5+_layout_top](./_media/Design_top.svg)  
Top layer design (also see [pin configurations](#pin-configurations))

![tofu5+_layout_bottom](./_media/Design_bot.svg)  
Bottom layer design
...
| Raspberry Pi header | Standard [40pin connector](https://pinout.xyz). ESD Protection on all GPIOs|
...
| M.2 Key B connector | The 2242 B key slot enables the use of SSDs, often with B+M key configuration, as well as network modules, which can then use the onboard micro SIM card. 1.5mm base height for the connected module. Network cards are more often available as 3042 format, which is also compatible. M.2 Key B connector provide a USB 2.0 + 3.0 bus and a PCIe Gen 2 bus (shared with the M.2 Key M connector, [switchable with DIP switch S1](#pin-configurations)).|
| M.2 Key M connector | The 2242 M key slot enables the use of SSD or other PCIe Gen 2 devices. 1.5mm base height for the connected module. M.2 Key M connector provide a PCIe Gen 2 bus (shared with the M.2 Key B connector, [switchable with DIP switch S1](#pin-configurations)). |
|I2C Temperature Sensor | [MCP9808](https://ww1.microchip.com/downloads/en/DeviceDoc/25095A.pdf) is a ±0.5°C Maximum Accuracy Digital Temperature Sensor, -20 to + 100 °C range IC. Use [CircuitPython MCP9808 Library](https://github.com/adafruit/Adafruit_CircuitPython_MCP9808) following this guide [Adafruit Guide to Use CircuitPython MCP9808 Library](https://learn.adafruit.com/adafruit-mcp9808-precision-i2c-temperature-sensor-guide/python-circuitpython). Or you can use directly this python code example located [here](https://github.com/DcubeTechVentures/MCP9808/blob/master/Python/MCP9808.py). |
| Cryptographic IC |[ATECC608B](https://ww1.microchip.com/downloads/en/DeviceDoc/ATECC608B-CryptoAuthentication-Device-Summary-Data-Sheet-DS40002239A.pdf) include protected storage for up to 16 keys, certificates or data and hardware cryptographic accelerator. Use [Official MicroChip CryptoAuthLib tools Library](https://github.com/MicrochipTech/cryptoauthtools) following this guide [Configuration Information for CryptoAuthLib tools](https://github.com/mgit-at/raspiECC). |
|Red, Green, Blue I2C LED | [150066M153000](https://www.we-online.com/components/products/datasheet/150066M153000.pdf?srsltid=AfmBOorSPdKJDQvm_jviOF-TNqu_fldxTljOBMLeUEN8UZIGZgq0YpIh) controlled by pin P0 - RED, P1 - GREEN and P2 - BLUE pins of I2C GPIO Expander [PCA9670](https://www.nxp.com/docs/en/data-sheet/PCA9670.pdf). Check the [Compatible PCA8574 CircuitPython Library](https://github.com/adafruit/Adafruit_CircuitPython_PCF8574)  and [Adafruit PCA8574 Use Guide](https://learn.adafruit.com/adafruit-pcf8574).   |
...
### eMMC boot
To boot on eMMC, hold down the nRPIBOOT button while plugging the USB-C cable for flashing (no other power input). The USB cable will power the CM5 while it boots. You may release the button after having plugged the cable. Then, you need to run the [rpiboot script](https://github.com/raspberrypi/usbboot). This can be done in two ways:
...
A neat benchmark test can be run to find your SSD performance by running the command from [pibenchmarks.com/](https://pibenchmarks.com/).
...
### NVMe SSD boot
Raspberry Pi supports NVMe SSD booting, and the TOFU5+ board is the perfect companion for that! You may find more information on Raspberry Pi's [documentation](https://www.raspberrypi.com/documentation/computers/raspberry-pi.html#nvme-ssd-boot).
...
### I2C temperature sensor
I2C temperature sensor is based on the [MCP9808](https://ww1.microchip.com/downloads/en/DeviceDoc/25095A.pdf) 

First, enable the I2C bus by adding the following parameter in the '/boot/firmware/config.txt' file
```
dtparam=i2c_arm=on
```

Use the following command to check if the I2C device is detected on the I2C0 bus:
```
i2cdetect -y 6
```
The device on address 0x18 shall be detected:

![I2C_address_detection_TEMP](./_media/I2C_address_detection_TEMP.png)

Clone the git repository with the example file.
```
git clone https://github.com/DcubeTechVentures/MCP9808.git
cd /MCP9808/Python/
```
Add missing parenthesis for the print functions at the end of [MCP9808.py](https://github.com/DcubeTechVentures/MCP9808/blob/master/Python/MCP9808.py) file
...
Launch the script and observe the output for successful temperature retrieval
```
python MCP9808.py
```
![TEMP_SUCCESS](./_media/TEMP_SUCCESS.png) 

### Cryptographic IC
Crpytographic IC is based on the [ATECC608B](https://ww1.microchip.com/downloads/en/DeviceDoc/ATECC608B-CryptoAuthentication-Device-Summary-Data-Sheet-DS40002239A.pdf)
...
i2cdetect -y 6
```
The device on address 0x60 shall be detected:

![I2C_address_detection_CRYPTO](./_media/I2C_address_detection_CRYPTO.png)

Then install the [Official MicroChip CryptoAuthLib tools Library](https://github.com/MicrochipTech/cryptoauthtools):
```
sudo apt install build-essential cmake libudev-dev libusb-1.0-0-dev libffi-dev libssl-dev git
git clone https://github.com/MicrochipTech/cryptoauthtools.git
cd cryptoauthtools/python/examples
pip install -r requirements.txt
```
Change the I2C address in the [info.py](https://github.com/MicrochipTech/cryptoauthtools/blob/master/python/examples/info.py) file at line 42 to 6
```
cfg.cfg.atcai2c.bus = 6
```
Launch the test script and observe the output for successful data retrieval 
```
python info.py -i i2c
```
![CRYPTO_SUCCESS](./_media/CRYPTO_SUCCESS.png) 

### RGB LED
RGB LED functionnality is based on the [150066M153000](https://www.we-online.com/components/products/datasheet/150066M153000.pdf?srsltid=AfmBOorSPdKJDQvm_jviOF-TNqu_fldxTljOBMLeUEN8UZIGZgq0YpIh) RGB LED and the I2C GPIO expander [PCA9670](https://www.nxp.com/docs/en/data-sheet/PCA9670.pdf).

First, enable the I2C bus by adding the following parameter in the '/boot/firmware/config.txt' file, then reboot
```
dtparam=i2c_arm=on
```
Use the following command to check if the I2C device is detected on the I2C0 bus:
...
i2cdetect -y 6
```
The device on address 0x10 shall be detected:

![I2C_address_detection_GPIO](./_media/I2C_address_detection_GPIO.png) 

You can use the following command to update the P0, P1 and P2 output and drive the RGB LED.

Drive LED White
```
i2cset -y 6 0x10 0x00 0
```

Drive LED Purple
```
i2cset -y 6 0x10 0x00 2
```

Drive LED Blue
```
i2cset -y 6 0x10 0x00 3
```

Drive LED Green
```
i2cset -y 6 0x10 0x00 4
```

Drive LED Red
```
i2cset -y 6 0x10 0x00 6
```

Drive LED OFF
```
i2cset -y 6 0x10 0x00 7
```
...
- GPIO output voltage selection, 3.3 V or 1.8 V. (populate only one resistor)
